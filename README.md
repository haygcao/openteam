...
团队状态由 `src/group/store.ts` 持久化，核心数据结构定义在 `src/group/types.ts`，包含：
...
src/content/
  ...
  frameHandshake.ts    iframe 角色绑定
  replyObserver.ts     回复监听、去重、轮询和超时补偿
  conversationMonitor.ts 会话 URL 监听与上报
...
src/group/
  store.ts             群聊状态持久化
  types.ts             群聊、人员、消息和 frame binding 类型
  mentionParser.ts     @人员 / @all 解析
  runtimeProtocol.ts   iframe content script 与 background 的运行时协议
